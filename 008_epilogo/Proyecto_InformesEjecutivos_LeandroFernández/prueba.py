...
def es_columna_numerica(columna):
    """
...
def analizar_y_limpiar_dataset(file_path):
    # Cargar el archivo CSV
    df = pd.read_csv(file_path)
    
    # Determinar qué columnas son numéricas
    columnas_numericas = [col for col in df.columns if es_columna_numerica(df[col])]
...
analizar_y_limpiar_dataset('amazon.csv')
